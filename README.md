...
以上です。**この時点ではまだ何も義務は発生しません。** 最初のチケットで `/flow` を呼んだときに、地図の初版生成から始まります([standard.md §12](standard.md#s12) の導入の順序)。
...
| 何を | 誰が |
|---|---|
| この変更が触る領域はどれか | ドメインオーナー |
| 生成された地図の初版を検収するか(通読して直す) | その領域のドメインオーナー |
...
- **地図の初版**は領域ごとに一度だけ。2回目以降のチケットでは飛びます
