...
| 何を | 誰が | 何をする |
|---|---|---|
| **期待値の書き出し** | 実装方針を書いていない人(レビュアー) | 仕様だけを見て、期待する入出力を表に書く。**実装と並列にやれるのでリードタイムは伸びない** |
| **一番怪しい箇所** | PR著者 | 自分の言葉で名指しする。選べないこと自体が理解不足のシグナル |
| **採らなかった選択肢** | PR著者 | 捨てた案と理由。設計メモからの転記ではなく、確定した判断を自分の言葉で |

この3つは**AIに埋めさせると意味が消えます**。「一番怪しい箇所」はAIが自分の出力を怪しいと思わないので情報になりません。期待値の書き出しは、実装を見た人が書くと自己照合になります。

### 全部が毎回走るわけではありません

- **地図に触るのはPR作成の段だけ**です。着手時には触りません。**初版は領域ごとに一度だけ**で、2回目以降は飛びます
- **独立照合**は不可逆系(スキーマ変更・データ移行・課金・外部送信・削除)に触るときだけ必須。契約理解レベルのコードには過剰装備です
...
- **社内ドキュメントに取り込む場合**
  > 本ドキュメントは「エージェント前提開発基準」(HawkClaws, CC BY 4.0) v0.10 に準拠しています。
