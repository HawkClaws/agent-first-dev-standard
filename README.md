...
| 生成された地図の初版を検収するか(初回のみ。最低ライン: Level 2・3 の通読と修正) | その領域のドメインオーナー |
...
この3つは**AIに生成させると意味が消えます**(記録の転記は例外——出所を明記した転記は生成ではありません)。PRの「一番怪しい箇所」は著者が書く欄ではありません——実装側の申告と設計メモの懸念点を機械が転記し、著者は違うものを消すだけです。理解検査の期待値は、エージェントに埋めさせた時点で検査ではなくなります。独立照合の期待値は、実装を見た人が書くと自己照合になります。
...
- **社内ドキュメントに取り込む場合**
  > 本ドキュメントは「エージェント前提開発基準」(HawkClaws, CC BY 4.0) v0.23 に準拠しています。
