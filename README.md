...
| **一番怪しい箇所** | PR著者 | 自分の言葉で名指しする。選べないこと自体が理解不足のシグナル |
| **採らなかった選択肢** | PR著者 | 捨てた案と理由。**本人の発言(会話ログ等)の逐語転記は可**(出所明記・どれを載せるかの選別は著者)。AIが生成した案の列挙や、整形・言い換えを伴う転記は不可 |

この4つは**AIに埋めさせると意味が消えます**。「一番怪しい箇所」はAIが自分の出力を怪しいと思わないので情報になりません。理解検査の期待値は、エージェントに埋めさせた時点で検査ではなくなります。独立照合の期待値は、実装を見た人が書くと自己照合になります。
...
- **社内ドキュメントに取り込む場合**
  > 本ドキュメントは「エージェント前提開発基準」(HawkClaws, CC BY 4.0) v0.20 に準拠しています。
