...
スキルを1本ずつ手で呼ぶ運用は、忙しい日に最初に消える。**`/flow <TICKET-ID>` は、機械でできる工程を連続実行し、人間の判断が必要な項目で止まる。** 状態はリポジトリの外(`~/.flow/<リポジトリ名>/<TICKET-ID>.md`)に持つので、人間が空欄を埋めてもう一度呼べば続きが走る。1チケットにつき**人間のゲートの数だけ**呼べばよい。
...
PR作成 ← 地図に触るのはここだけ
  └─ 👤 この変更が触る領域を決める(選択肢が実質1つなら機械が決めて人が検査。[§7](../standard.md#s7))
...
人間レビュー
  └─ /crosscheck     … 独立照合の**人間版**の突き合わせと裁定(**不可逆系は必須**。期待値を書くのは実装方針を書いていない人。[§4](../standard.md#s4))
マージ(人間の決断)
