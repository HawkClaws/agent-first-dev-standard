...
スキルを1本ずつ手で呼ぶ運用は、忙しい日に最初に消える。**`/flow <TICKET-ID>` は、機械でできる工程を連続実行し、人間の判断が必要な**停止**でまとめて聞く。** 停止は標準ケースで**3回**(着手/PR確認/マージ。実装が着手時の規模の上限を超えて停止1に戻れば1回増える——👤 は増えない)——👤 ゲートの数は減らさず、聞く場所を束ねてある。状態はリポジトリの外(`~/.flow/<リポジトリ名>/<TICKET-ID>.md`)に持つので、人間が答えてもう一度呼べば続きが走る(実装完了後の継続呼び出しは判断ゼロ。完了通知が届く環境では自動)。
...
flowchart TD
  ...
  subgraph S1["停止1: 着手(1回の停止でまとめて聞く)"]
    direction TB
    dm["👤 /design-memo 設計の聞き取り(著者。価値の見立て→規模の上限→設計。1問ずつ、曖昧さが尽きるまで)"]:::human
    tc["/design-memo チーム確認事項の推奨(不可逆・契約や境界・他人の領域・決められなかった点・釣り合わない規模)"]:::machine
    g1["👤 チーム確認事項への回答(決める人1名。0件・不可逆系なしなら無し。不可逆系は明示承認)"]:::human
    hd["ヘッダへの申告: 設計メモの在り処と確認・実データの所在・結合テストの実行手段・規模の上限(設計メモから写す)"]:::ask
  ...
  subgraph C["継続呼び出し(機械)"]
    direction TB
    done["完了確認(結合テストの有無・結果。規模超過なら停止1へ戻る)"]:::machine
    ...
    fr["/first-review AI一次レビュー(別セッション。出力はファイル、戻り値はパスのみ)"]:::machine
