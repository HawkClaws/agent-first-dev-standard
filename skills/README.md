...
停止1: 着手(1回の停止でまとめて聞く)
  ...
  └─ /crosscheck 手順0 … 検査の段と版の判定。計測・集計系なら実データの所在、結合テストの実行手段もここで確認(知識の申告。👤 には数えない)
  └─ 実装をサブエージェントへ委譲(**範囲は実装・単体テスト・結合テストまで**。[§4](../standard.md#s4)「委譲の範囲」。**送信のみ。完了は待たない**)
  └─ /crosscheck 手順B-1 … 独立照合の**自動版の生成**(委譲の直後=構成保証)。出力はテストコードが既定。機械が回せない境界・納品や監査が要る場合は**結合テスト仕様書**
...
(実装の完了待ち → 継続呼び出し。判断ゼロ)
  ...
  └─ /crosscheck    … 理解検査の突き合わせ。種別(A/B)は**提案**まで。自動版の**テストコードの実行**もここ(結合テスト仕様書は停止2で著者が実行)
...
停止2: PR確認(1回の停止でまとめて聞く。**記録が済むまで first-review の出力は開かない**)
  └─ 完了報告の「迷った箇所」(結合テストの未実施/対象外の判定も)を著者が読む → 👤 種別(A/B)の確定とBの反映先の指定(探すのは著者)→ 「固定した/しなかった境界」を読む(種別確定の後)
  └─ 結合テスト仕様書の実行と結果の記入(著者。自動版の出力が仕様書のとき。観測を書いてから一致/食い違い。判断ではないので 👤 には数えないが作業時間がかかる)
