...
[基準本文](../standard.md)の運用部分を、コーディングエージェントのスキル(手順書)として実装したもの。基準は覚えて守るものではなく、`/flow` と打てば始まるものにする。
...
flowchart TD
  ...
  subgraph S1["停止1: 着手(1回の停止でまとめて聞く)"]
    direction TB
    dm["👤 /design-memo 設計の聞き取り(著者。1問ずつ、曖昧さが尽きるまで)"]:::human
    tc["/design-memo チーム確認事項の推奨(不可逆・契約や境界・他人の領域・決められなかった点)"]:::machine
    g1["👤 チーム確認事項への回答(0件・不可逆系なしなら無し。不可逆系は明示承認)"]:::human
    hd["ヘッダへの申告: 設計メモの在り処と確認・実データの所在・結合テストの実行手段"]:::ask
    c0["/crosscheck 手順0 不可逆系なら独立照合の版を判定"]:::machine
  ...
  subgraph C["継続呼び出し(機械)"]
    ...
    cc["メモ外の決定の列挙(完了報告から転記)・/crosscheck 自動版テストコードの実行"]:::machine
  ...
  subgraph S2["停止2: PR確認(提示順を守る。記録が済むまで first-review を開かない)"]
    direction TB
    rd["完了報告の「メモ外の決定」「迷った箇所」を読む"]:::ask
    sp["結合テスト仕様書の実行と記入(著者。仕様書のときのみ)"]:::ask
    g3["👤 メモ外の決定の仕分け(受け入れ→反映先 / 差し戻し / 裁定へ)"]:::human
    ...
    post["受け入れの転記・差し戻しの修正・PR欄の再生成・欄の突き合わせ検査"]:::machine
  ...
  subgraph R["人間レビュー(独立照合をやる場合のみ)"]
    ...
    g8["👤 食い違いの裁定(仕様の持ち主。「裁定へ」の決定も)"]:::human
...
スキルが自動化するのは**準備・機械検査・判断の候補提示まで**。判断——設計の聞き取りの答え、チーム確認事項への回答、メモ外の決定の仕分け、「採らなかった選択肢」の選別、独立照合の食い違いの裁定、理解レベルの確定、マージの決断——は、出力の**空欄**として人間に残す。

ただし、**担当は「著者か機械か」の2区分では決まらない。欄や工程が知識と判断のどちらに基づくかで決まる**(基準[§7](../standard.md#s7)・[§9](../standard.md#s9)):

| 種類 | 担当 | 例 |
|---|---|---|
| 作業から得られる知識 | **やった側**(実装したエージェントの申告、結合テスト仕様書を実行した著者の観測) | 完了報告の「メモ外の決定」「迷った箇所」(著者が停止2の先頭で読み、メモ外の決定を仕分ける)・「結合テストで固定した境界・固定しなかった境界」(仕分けの後で読む) |
| 記録からの転記 | **機械**(生成は不可のまま。可否を人に聞かない——聞くこと自体が人を止める) | 「採らなかった選択肢」の候補・聞き取りの答えの書き留め・チーム確認事項の推奨(何を聞くかの候補) |
