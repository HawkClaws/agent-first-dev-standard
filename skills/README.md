...
スキルを1本ずつ手で呼ぶ運用は、忙しい日に最初に消える。**`/flow <TICKET-ID>` は、機械でできる工程を連続実行し、人間の判断が必要な項目で止まる。** 状態は `docs/flow/<TICKET-ID>.md` に持つので、人間が空欄を埋めてもう一度呼べば続きが走る。1チケットにつき**人間のゲートの数だけ**呼べばよい。
